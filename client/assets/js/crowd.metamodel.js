...
CrowdMetamodel.prototype.request = function (req) {
  ...
  if (req.repair) {
    //first we have to request the kf to owl endpoint for get the ontology for meta repair
    var owlReq = {
      from: 'kf',
      to: 'owl',
      data: req.meta
    ...
    self.request({
      success: function (res) {
        if (req.success) {
          ...
          formData.append('meta', JSON.stringify(req.meta));
          formData.append('entity', req.entity);
          formData.append('reasoner', req.reasoner);
          formData.append('maxExplanations', req.maxExplanations ? req.maxExplanations : 5);
          formData.append('precompute', req.precompute ? req.precompute : false);
          formData.append('filtering', req.filtering ? req.filtering : true);
          //define the ontology based on the response of the meta to owl request
          formData.append('ontology', res);
          ...
          $.ajax({
            ...
            success: function (res) {
              console.log('MetamodelAPI: response to ' + url + 'metarepair');
