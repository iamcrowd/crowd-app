...
CrowdEditor.prototype.init = function () {
  ...
  if (!self.config.conceptualModel.initPalette) {
    self.config.conceptualModel.initPalette = function () {
      console.warn("CrowdEditor conceptual model palette is not defined.");
    };
  }
  if (!self.config.conceptualModel.initElementsToolsViews) {
    self.config.conceptualModel.initElementsToolsViews = function () {
      console.warn(
        "CrowdEditor conceptual model elements tools views are not defined."
      );
    };
  }
  if (!self.config.conceptualModel.initLinksToolsViews) {
    self.config.conceptualModel.initLinksToolsViews = function () {
      console.warn(
        "CrowdEditor conceptual model links tools views are not defined."
      );
    };
  }
  if (!self.config.conceptualModel.initChangeAttributesEvents) {
    self.config.conceptualModel.initChangeAttributesEvents = function () {
      console.warn(
        "CrowdEditor conceptual model change attributes events are not defined."
      );
    };
  }
  if (!self.config.conceptualModel.initInspector) {
    self.config.conceptualModel.initInspector = function () {
      console.warn("CrowdEditor conceptual model inspector is not defined.");
    };
  }
  if (!self.config.conceptualModel.initSyntaxValidator) {
    self.config.conceptualModel.initSyntaxValidator = function () {
      console.warn(
        "CrowdEditor conceptual model syntax validator is not defined."
      );
    };
  }
  if (!self.config.conceptualModel.initReasoningValidator) {
    self.config.conceptualModel.initReasoningValidator = function () {
      console.warn(
        "CrowdEditor conceptual model reasoning validator is not defined."
      );
    };
  }
  if (!self.config.conceptualModel.initRepairingTools) {
    self.config.conceptualModel.initRepairingTools = function () {
      console.warn(
        "CrowdEditor conceptual model repairing tools are not defined."
      );
    };
  }
  if (!self.config.conceptualModel.fromJSONSchema) {
    self.config.conceptualModel.fromJSONSchema = function () {
      console.warn(
        "CrowdEditor conceptual model fromJSONSchema is not defined."
      );
    };
  }
  if (self.config.reasoningApi && !self.config.conceptualModel.fromReasoning) {
    self.config.conceptualModel.fromReasoning = function () {
      console.warn(
        "CrowdEditor conceptual model fromReasoning is not defined and the Reasoning API is active."
      );
    };
  }
  if (self.config.metamodelApi && !self.config.conceptualModel.fromRepairMark) {
    self.config.conceptualModel.fromRepairMark = function () {
      console.warn(
        "CrowdEditor conceptual model fromRepairMark is not defined and the Metamodel API is active."
      );
    };
  }
  if (
    self.config.metamodelApi &&
    !self.config.conceptualModel.fromRepairApply
  ) {
    self.config.conceptualModel.fromRepairApply = function () {
      console.warn(
        "CrowdEditor conceptual model fromRepairApply is not defined and the Metamodel API is active."
      );
    };
  }
  ...
  $("#" + self.config.selector).append(
    '<div class="crowd-container" id="crowd-container-' + self.id + '"></div>'
  );

  //append dom element for palette
  $("#crowd-container-" + self.id).append(
    '<div class="crowd-palette" id="crowd-palette-' + self.id + '"></div>'
  );

  //append dom element that contains the tools and the workspace
  $("#crowd-container-" + self.id).append(
    '<div class="crowd-container-middle" id="crowd-container-middle-' +
      self.id +
      '"></div>'
  );
  ...
  $("#crowd-container-" + self.id).append(
    '<div class="crowd-container-side-toggle" id="crowd-container-side-toggle-' +
      self.id +
      '" \
  ...
  $("#crowd-container-" + self.id).append(
    '<div class="crowd-container-side" id="crowd-container-side-' +
      self.id +
      '"></div>'
  );

  //append dom element for tools
  $("#crowd-container-middle-" + self.id).append(
    '<div class="crowd-tools" id="crowd-tools-' + self.id + '"></div>'
  );

  //append dom element for workspace
  $("#crowd-container-middle-" + self.id).append(
    '<div class="crowd-workspace" id="crowd-workspace-' + self.id + '"></div>'
  );

  //append dom element for inspector
  $("#crowd-container-side-" + self.id).append(
    '<div class="crowd-inspector" id="crowd-inspector-' + self.id + '"></div>'
  );

  //append dom element for map
  $("#crowd-container-side-" + self.id).append(
    '<div class="crowd-map" id="crowd-map-' + self.id + '"></div>'
  );

  //append dom element for side toggle icon
  $("#crowd-container-side-toggle-" + self.id).append(
    '<i id="crowd-container-side-toggle-icon-' +
      self.id +
  ...
  $("#crowd-container-side-toggle-" + self.id).on("click", function () {
    ...
    $("#crowd-container-side-toggle-icon-" + self.id).toggleClass(
      "fa-chevron-right"
    );
    $("#crowd-container-side-toggle-icon-" + self.id).toggleClass(
      "fa-chevron-left"
    ...
    self.enumerate.getNumber = function (cell, attr) {
      ...
      self.workspace.graph.getCells().forEach(function (existentCell) {
        if (existentCell.prop(attr)?.indexOf(cell.prop(attr) + "-") != -1) {
          var number = parseInt(
            existentCell.prop(attr)?.split(cell.prop(attr) + "-")[1]
          );
          if (!isNaN(number)) {
        ...
        cell.prop("enumerate").forEach(function (attrEnum) {
          if (cell.prop(attrEnum))
            cell.prop(
              ...
              cell.prop(attrEnum) +
                "-" +
                self.enumerate.getNumber(cell, attrEnum)
            );
        });
      }
    };
  ...
  //initialize namespaces for use in uris
...
CrowdEditor.prototype.initPalette = function () {
  ...
  $("#crowd-palette-" + self.id).css(
    "width",
    self.config.palette.grid.width * self.config.palette.grid.columns + 1
  );
  $("#crowd-container-middle-" + self.id).css(
    "left",
    self.config.palette.grid.width * self.config.palette.grid.columns
  );
  ...
  self.palette.paper = new joint.dia.Paper({
    ...
    height:
      (Object.keys(self.palette.elements).length /
        self.config.palette.grid.columns +
        5) *
      self.config.palette.grid.height, //$('#crowd-palette-' + self.id).height(),
    ...
    // },
    gridSize:
      self.config.palette.grid.height > self.config.palette.grid.width
        ? self.config.palette.grid.height
        : self.config.palette.grid.width,
    // drawGrid:
    // {
    //   name: 'mesh',
    //   args: [
  ...
  if ($("#crowd-palette-" + self.id + " svg")[0])
    $("#crowd-palette-" + self.id + " svg")[0].setAttribute(
      "height",
      (Object.keys(self.palette.elements).length /
        self.config.palette.grid.columns +
        1) *
  ...
  for (var element in self.palette.elements) {
    self.palette.elements[element].attributes.position = {
      x:
        (position % self.config.palette.grid.columns) *
          self.config.palette.grid.width +
        (self.config.palette.grid.width -
          self.palette.elements[element].attributes.size.width) /
          2,
      y:
        Math.floor(position / self.config.palette.grid.columns) *
          self.config.palette.grid.height +
        (self.config.palette.grid.height -
          self.palette.elements[element].attributes.size.height) /
          2,
    };
  ...
  self.palette.paper.on("cell:pointerdown", function (cellView, e) {
    ...
    $("body").append(
      '<div class="crowd-flypaper" id="crowd-flypaper-' + flyid + '"></div>'
    );
    ...
    var offset = {
      ...
      y:
        (flyShape.attributes.size.height / 2) * self.workspace.paper.scale().sy,
    ...
    $("body").on("mouseup.fly", function (e) {
      ...
      if ($("#crowd-workspace-scrollable-" + self.id + ":hover").length) {
        ...
        if (!$("#crowd-workspace-paper-" + self.id + ":hover").length) {
          ...
          var paperPos = {
            x:
              $("#crowd-workspace-paper-" + self.id).offset().left -
              $("#crowd-workspace-scrollable-" + self.id).offset().left,
            y:
              $("#crowd-workspace-paper-" + self.id).offset().top -
              $("#crowd-workspace-scrollable-" + self.id).offset().top,
          };
        ...
        var p = {
          x:
            (relativePos.x - self.workspace.paper.translate().tx) /
            self.workspace.paper.scale().sx,
          y:
            (relativePos.y - self.workspace.paper.translate().ty) /
            self.workspace.paper.scale().sy,
        };
...
CrowdEditor.prototype.initTools = function () {
  ...
  $("#crowd-tools-" + self.id).append(
    '<span class="row" id="crowd-tools-row-' + self.id + '"></span>'
  );

  //append dom for menu options
  $("#crowd-tools-row-" + self.id).append(
    '<div class="form-group"><div class="btn-group" id="crowd-tools-menu-' +
      self.id +
      '" role="group"></div></div>'
  );
  ...
  self.tools.file.init = function () {
    ...
    self.tools.file.updateActualFile = function (config) {
      ...
      if (config?.parent || config?.parent == null)
        $(config?.parent ? "#" + config?.parent : ".modal").modal("hide");
    };
        ...
        self.toBase64({ miniature: true }, function (preview) {
          ...
          self.tools.export.exportTo({
            ...
            //finally do the callback with or without the meta
              ...
              callback(
                self.config.actualFile,
                data.statusText == "error" ? data : null
              );
            },
            hideError: true,
          });
    ...
    $("#crowd-tools-menu-" + self.id).append(
      '<div class="btn-group dropdown" id="crowd-tools-file-btn-' +
        self.id +
        '"> \
        <button class="btn btn-adaptive dropdown-toggle" type="button" id="crowd-tools-file-dropdown-' +
        self.id +
        '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
          <i class="fa fa-fw fa-file"></i> File \
        </button> \
        <ul class="dropdown-menu" aria-labelledby="crowd-tools-file-dropdown-' +
        self.id +
        '"></ul> \
      </div>'
    );
    ...
    $("#crowd-workspace-" + self.id).append(
      '<div class="crowd-tools-actual-file-container"> \
        ...
        data-toggle="tooltip" data-original-title="Diagram loaded from Cloud" data-placement="right"> \
          ...
          <span id="crowd-tools-actual-file-' +
        self.id +
        '"></span> \
        </label> \
      </div>'
    ...
    self.tools.new.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        ...
        title="' +
          (!self.config?.ngFiles?.user
            ? loggedInMessage
            : "Create New Diagram") +
          '" > \
          <button class="dropdown-item" id="crowd-tools-file-new-input-' +
          self.id +
          '" \
          ' +
          (!self.config?.ngFiles?.user
      ...
      $("body").append(
        '<div id="crowd-tools-file-new-advertisement-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog"> \
            <div class="modal-content"> \
              ...
              <div class="modal-footer"> \
                ...
                <button id="crowd-tools-file-new-advertisement-proceed-' +
          self.id +
          '" \
      ...
      $("#crowd-tools-file-new-advertisement-proceed-" + self.id).on(
        "click",
        function () {
    ...
    self.tools.load.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        ...
        title="' +
          ...
          '" > \
          <button class="dropdown-item" id="crowd-tools-load-input-' +
          self.id +
          '" \
          ' +
          (!self.config?.ngFiles?.user
    ...
    self.tools.save.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        ...
        title="' +
          (!self.config?.ngFiles?.user
            ? loggedInMessage
            : "Save Diagram on Cloud") +
          '" > \
          <button class="dropdown-item" id="crowd-tools-save-input-save-' +
          self.id +
          '" \
          ' +
          (!self.config?.ngFiles?.user
        ...
        title="' +
          (!self.config?.ngFiles?.user
            ? loggedInMessage
            : "Save Diagram on Cloud as ...") +
          '" > \
          <button class="dropdown-item" id="crowd-tools-save-input-saveas-' +
          self.id +
          '" \
          ' +
          (!self.config?.ngFiles?.user
    ...
    self.tools.rename.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        ...
        <li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" \
          title="' +
          (!self.config?.ngFiles?.user ? loggedInMessage : "Rename Diagram") +
          '" > \
            <button class="dropdown-item" id="crowd-tools-rename-input-' +
          self.id +
          '" \
            ' +
          (!self.config?.ngFiles?.user
    ...
    self.tools.delete.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" \
          title="' +
          (!self.config?.ngFiles?.user ? loggedInMessage : "Delete Diagram") +
          '" > \
            <button class="dropdown-item" style="color: #dc3545;" id="crowd-tools-delete-input-' +
          self.id +
          '" \
            ' +
          (!self.config?.ngFiles?.user
    ...
    self.tools.export.init = function () {
      ...
      self.tools.export.exportTo = function (options) {
        if (
          self.config.conceptualModel.name ==
          self.config.availableConceptualModels[options?.model].name
        ) {
          if (options?.success)
            options.success(self.config.conceptualModel.toJSONSchema(self));
          if (options?.finally)
            options.finally(self.config.conceptualModel.toJSONSchema(self));
        } else {
          self.config.metamodelApi.request({
            ...
            success: function (response) {
              if (
                ...
                self.config.metamodelApi.request({
                  from: "kf",
                  to: self.config.availableConceptualModels[options?.model]
                    .name,
                  data: response,
      ...
      self.tools.export._owlExport = function (download) {
        var encoding = $(
          "#crowd-tools-export-options-encoding-" + self.id + " option:selected"
        ).val();
        var syntax = $(
          "#crowd-tools-export-options-syntax-" + self.id + " option:selected"
        ).val();
        if (encoding == "owllink-alcqi")
          $("#crowd-tools-export-options-syntax-menu-" + self.id).hide();
        else $("#crowd-tools-export-options-syntax-menu-" + self.id).show();

        $("#crowd-tools-export-options-encoding-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-options-syntax-" + self.id).prop(
          "disabled",
          true
        );
        ...
        self.tools.export.exportTo({
          ...
          success: function (schema) {
            $(
              "#crowd-tools-export-check-schema-modal-" +
                self.id +
                " .modal-body pre"
            ).html(escapeXML(formatXML(schema)));
            ...
            $(
              '[aria-labelledby="crowd-tools-export-dropdown-' + self.id + '"]'
            ).dropdown("hide");
            $("#crowd-tools-export-options-encoding-" + self.id).prop(
              "disabled",
              false
      ...
      self.tools.export._verbalizationExport = function (download) {
        var verbalization = $(
          "#crowd-tools-export-options-verbalization-" +
            self.id +
            " option:selected"
        ).val();
        var language = $(
          "#crowd-tools-export-options-language-" + self.id + " option:selected"
        ).val();

        $("#crowd-tools-export-options-verbalization-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-options-language-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-loading-" + self.id).show();
      ...
      self.tools.export._imageExport = function (download) {
        var format = $(
          "#crowd-tools-export-options-format-" + self.id + " option:selected"
        ).val();
        var useZoom = $("#crowd-tools-export-options-use-zoom-" + self.id).is(
          ":checked"
        );
        var useGrid =
          format == "pdf"
            ? false
            : $("#crowd-tools-export-options-use-grid-" + self.id).is(
                ":checked"
              );
        var onlyContent = $(
        ...
        if (format == "pdf")
          $("#crowd-tools-export-options-use-grid-menu-" + self.id).hide();
        else $("#crowd-tools-export-options-use-grid-menu-" + self.id).show();

        $("#crowd-tools-export-options-use-zoom-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-options-use-grid-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-options-only-content-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-options-format-" + self.id).prop(
          "disabled",
          true
        );
        $("#crowd-tools-export-loading-" + self.id).show();
      ...
      self.tools.export._download = function (options) {
        if (options.format == "pdf") {
          ...
          var pdfSettings = {
            name: "image",
            pageWidth: options.onlyContent
              ? content.width + self.workspace.paperPadding * 2
              : svg.width(),
            pageHeight: options.onlyContent
              ? content.height + self.workspace.paperPadding * 2
              : svg.height(),
            x: options.onlyContent
              ? -(content.x - self.workspace.paperPadding)
              : 0,
            y: options.onlyContent
              ? -(content.y - self.workspace.paperPadding)
              : 0,
            width: svg.width(),
            height: svg.height(),
          };
          ...
          $("<a />", {
            download:
              options.name + (options.format ? "." + options.format : ""),
            href: options.href
              ? options.href
              : "data:" +
                options.type +
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        '<li class="dropdown-divider"></li> \
        <li class="dropdown"> \
          <button class="dropdown-item dropdown-toggle" type="button" id="crowd-tools-export-dropdown-' +
          self.id +
          '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            <i class="fa fa-fw fa-download"></i> Export \
          </button> \
          <ul class="dropdown-menu" aria-labelledby="crowd-tools-export-dropdown-' +
          self.id +
          '"></ul> \
        </li>'
      );
      ...
      $.each(
        self.config.availableConceptualModels,
        function (conceptualModelName, conceptualModel) {
          if (conceptualModel.export == null || conceptualModel.export) {
            var title = conceptualModel.title
              ? conceptualModel.title
              : conceptualModelName.toUpperCase();
            $(
              '[aria-labelledby="crowd-tools-export-dropdown-' + self.id + '"]'
            ).append(
              '<li><button class="dropdown-item" data-model="' +
                conceptualModelName +
                '" name="crowd-tools-export-check-schema-' +
                self.id +
                '">' +
      ...
      $("body").append(
        '<div id="crowd-tools-export-check-schema-modal-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog modal-dialog-scrollable modal-lg"> \
            <div class="modal-content"> \
              <div class="modal-header"> \
                ...
                <div class="spinner-border text-primary" role="status" id="crowd-tools-export-loading-' +
          self.id +
          '" style="display: none"> \
              ...
              <div class="modal-body"> \
              <div id="crowd-tools-export-owl-options-' +
          self.id +
          '" style="display: none"> \
                <div class="row"> \
                  <div class="form-group col-6"> \
                    <label class="" for="">Encoding</label> \
                    <select class=" form-control custom-select my-1 mr-sm-2" id="crowd-tools-export-options-encoding-' +
          self.id +
          '"> \
                  ...
                  <div class="form-group col-6" id="crowd-tools-export-options-syntax-menu-' +
          self.id +
          '"> \
                    <label class="" for="">Syntax</label> \
                    <select class=" form-control custom-select my-1 mr-sm-2" id="crowd-tools-export-options-syntax-' +
          self.id +
          '"> \
              ...
              <div id="crowd-tools-export-verbalization-options-' +
          self.id +
          '" style="display: none"> \
                <div class="row"> \
                  <div class="form-group col-6"> \
                    <label class="" for="">Verbalization</label> \
                    <select class=" form-control custom-select my-1 mr-sm-2" id="crowd-tools-export-options-verbalization-' +
          self.id +
          '"> \
                  ...
                  <div class="form-group col-6" id="crowd-tools-export-options-language-menu-' +
          self.id +
          '"> \
                    <label class="" for="">Language</label> \
                    <select class=" form-control custom-select my-1 mr-sm-2" id="crowd-tools-export-options-language-' +
          self.id +
          '"> \
              ...
              <div id="crowd-tools-export-image-options-' +
          self.id +
          '" style="display: none"> \
                <div class="row"> \
                  <div class="form-group col-6"> \
                    <label class="" for="">Format</label> \
                    <select class=" form-control custom-select my-1 mr-sm-2" id="crowd-tools-export-options-format-' +
          self.id +
          '"> \
                  ...
                  <div class="form-group col-3"> \
                    <div class="form-group form-check"> \
                      <input type="checkbox" class="form-check-input" id="crowd-tools-export-options-use-zoom-' +
          self.id +
          '"> \
                      <label class="form-check-label" for="crowd-tools-export-options-use-zoom-' +
          self.id +
          '">Use Zoom</label> \
                    </div>\
                    <div class="form-group form-check" id="crowd-tools-export-options-use-grid-menu-' +
          self.id +
          '"> \
                      <input type="checkbox" class="form-check-input" id="crowd-tools-export-options-use-grid-' +
          self.id +
          '"> \
                      <label class="form-check-label" for="crowd-tools-export-options-use-grid-' +
          self.id +
          '">Use Grid</label> \
                    </div>\
                  </div> \
                  <div class="form-group col-3"> \
                    <div class="form-group form-check"> \
                      <input type="checkbox" class="form-check-input" id="crowd-tools-export-options-only-content-' +
          self.id +
          '" checked> \
                      <label class="form-check-label" for="crowd-tools-export-options-only-content-' +
          self.id +
          '">Only Content</label> \
                    </div>\
                  </div> \
              ...
              <pre id="crowd-tools-export-check-schema-modal-pre-' +
          self.id +
          '" \
              style="overflow: hidden; overflow-wrap: break-word; white-space: pre-wrap;"></pre> \
              <div style="width: 100%; text-align: center;">\
                <img id="crowd-tools-export-check-schema-modal-img-' +
          self.id +
          '" \
              style="max-width: 100%;" > \
              </div> \
              ...
              <div class="modal-footer"> \
                ...
                <button class="btn btn-adaptive" data-clipboard-target="#crowd-tools-export-check-schema-modal-pre-' +
          self.id +
          '"> \
                  Copy to Clipboard \
                </button> \
                <button class="btn btn-adaptive" data-model="" id="crowd-tools-export-schema-' +
          self.id +
          '">Download</button> \
              </div> \
            </div> \
      ...
      $(
        "#crowd-tools-export-options-encoding-" +
          self.id +
          ", #crowd-tools-export-options-syntax-" +
          self.id
      ).change(function () {
      ...
      $(
        "#crowd-tools-export-options-verbalization-" +
          self.id +
          ", #crowd-tools-export-options-lanaguage-" +
          self.id
      ).change(function () {
      ...
      $(
        "#crowd-tools-export-options-use-zoom-" +
          self.id +
          ", #crowd-tools-export-options-use-grid-" +
          self.id +
          ", #crowd-tools-export-options-only-content-" +
          self.id +
          ", #crowd-tools-export-options-format-" +
          self.id
      ).change(function () {
      ...
      $('[name="crowd-tools-export-check-schema-' + self.id + '"]').on(
        "click",
        function (event) {
          var btn = this;
          ...
          $("#crowd-tools-export-check-schema-modal-img-" + self.id).attr(
            "src",
            ""
          );
          ...
          if (model == "owl") {
            ...
            $(
              ...
              '<i class="fa fa-fw fa-download"></i> Export ' + modelObj.title
            );
            $("#crowd-tools-export-check-schema-modal-" + self.id).modal(
              "show"
            );
            self.tools.export._owlExport();
            ...
            self.tools.export.exportTo({
              ...
              success: function (schema) {
                ...
                $(
                  '[aria-labelledby="crowd-tools-export-dropdown-' +
                    self.id +
                    '"]'
                ).dropdown("hide");
              },
    ...
    self.tools.import.init = function () {
      ...
      self.tools.import.importFrom = function (diagram) {
          ...
          self.config.metamodelApi.request({
            ...
            success: (response) => {
              ...
              self.tools.import.importFrom({
                model: self.config.conceptualModel.name,
                schema: response,
        ...
        if (diagram.model == self.config.conceptualModel.name) {
          if (
            self.hasChanges() &&
            (diagram.forced == null || !diagram.forced)
          ) {
        ...
        //in case that actual conceptual model was different, redirect to url of the correct model sending the diagram schema as parameter
          ...
          if (
            ...
            if (
              self.config.tools &&
              self.config.tools.import &&
              self.config.tools.import.errors &&
              self.config.tools.import.errors.missingModelPalette
            )
              self.config.tools.import.errors.missingModelPalette(
                diagram.model
              );
              ...
              alert(
                "There is no palette for the conceptual model " +
                  model.toUpperCase() +
                  "."
              );
          }
        ...
        '<div id="crowd-tools-import-advertisement-' +
          ...
          <div class="modal-dialog"> \
            <div class="modal-content"> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> \
                <button id="crowd-tools-import-advertisement-proceed-' +
          self.id +
          '" \
                type="button" class="btn btn-danger" data-dismiss="modal">Proceed</button> \
              </div> \
      ...
      $("#crowd-tools-import-advertisement-proceed-" + self.id).on(
        "click",
        function () {
          self.tools.import.importFromLocal();
          $(".tooltip").tooltip("hide");
      ...
      $('[aria-labelledby="crowd-tools-file-dropdown-' + self.id + '"]').append(
        '<li class="dropdown"> \
          <button class="dropdown-item dropdown-toggle" type="button" id="crowd-tools-import-dropdown-' +
          self.id +
          '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            <i class="fa fa-fw fa-upload"></i> Import \
          </button> \
          <ul class="dropdown-menu" aria-labelledby="crowd-tools-import-dropdown-' +
          self.id +
          '"></ul> \
        </li>'
      );
      ...
      $.each(
        ...
        function (conceptualModelName, conceptualModel) {
          ...
          if (conceptualModel.import == null || conceptualModel.import) {
            $(
              '[aria-labelledby="crowd-tools-import-dropdown-' + self.id + '"]'
            ).append(
              '<li><button class="dropdown-item" data-model="' +
                conceptualModelName +
                '" name="crowd-tools-import-check-schema-' +
                self.id +
                '">' +
      ...
      $("body").append(
        '<div id="crowd-tools-import-check-schema-modal-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog modal-dialog-scrollable modal-lg"> \
            <div class="modal-content"> \
          ...
          self.id +
          '">Select a schema file</label> \
                  <div class="custom-file"> \
                    <input type="file" class="custom-file-input" id="crowd-tools-import-file-' +
          self.id +
          '"> \
                    <label class="custom-file-label" for="crowd-tools-import-file-' +
          self.id +
          '">Choose file</label> \
                  </div> \
                </div> \
          ...
          '" rows="10"></textarea> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button> \
                <button class="btn btn-adaptive" data-model="" id="crowd-tools-import-schema-' +
          self.id +
          '">Upload</button> \
              </div> \
            </div> \
      ...
      $("#crowd-tools-import-file-" + self.id).on("change", function () {
        ...
        if (this.files[0])
          fileName = document.getElementById(
            "crowd-tools-import-file-" + self.id
          ).files[0].name;
      ...
      $('[name="crowd-tools-import-check-schema-' + self.id + '"]').on(
        "click",
        function (event) {
          // var btn = this;
          event.stopPropagation();
          ...
          $(
            "#crowd-tools-import-check-schema-modal-" +
              self.id +
              " .modal-title"
          ).html(
            '<i class="fa fa-fw fa-upload"></i> Import ' +
      ...
      $("#crowd-tools-import-check-schema-modal-" + self.id).on(
        "hidden.bs.modal",
        function () {
  ...
  self.tools.edit.init = function () {
    ...
    $("#crowd-tools-menu-" + self.id).append(
      '<div class="btn-group dropdown" id="crowd-tools-edit-btn-' +
        self.id +
        '"> \
        <button class="btn btn-adaptive dropdown-toggle" type="button" id="crowd-tools-edit-dropdown-' +
        self.id +
        '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
          <i class="fa fa-fw fa-pencil"></i> Edit \
        </button> \
        <ul class="dropdown-menu" aria-labelledby="crowd-tools-edit-dropdown-' +
        self.id +
        '"></ul> \
      </div>'
    );
    ...
    self.tools.layout.init = function () {
      ...
      self.tools.layout.doLayout = function (options) {
        ...
        setTimeout(() => {
          ...
          var filteredGraph = self.workspace.graph
            .getSubgraph(self.workspace.graph.getCells())
            .filter(function (cell) {
              return (
                !cell.prop("layoutIgnore") &&
                (!cell.isLink() ||
                  (!cell.getSourceCell()?.prop("layoutIgnore") &&
                    !cell.getTargetCell()?.prop("layoutIgnore") &&
                    !cell.getSourceCell()?.isLink() &&
                    !cell.getTargetCell()?.isLink()))
              );
            });
      ...
      $('[aria-labelledby="crowd-tools-edit-dropdown-' + self.id + '"]').append(
        ...
        // </li>'
        '<li class="dropdown"> \
          <button class="dropdown-item dropdown-toggle" type="button" id="crowd-tools-layout-dropdown-' +
          self.id +
          '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            <i class="fa fa-fw fa-sitemap"></i> Layout \
          </button> \
          <ul class="dropdown-menu" aria-labelledby="crowd-tools-layout-dropdown-' +
          self.id +
          '"></ul> \
        </li>'
      );
      ...
      $.each(
        ...
        function (index, algorithm) {
          $(
            '[aria-labelledby="crowd-tools-layout-dropdown-' + self.id + '"]'
          ).append(
            '<button class="dropdown-item" data-algorithm="' +
              algorithm +
              '" name="crowd-tools-layout-algorithm-' +
              self.id +
              '">' +
              '<i class="fa fa-fw fa-long-arrow-right"></i> ' +
              upperFirstLetter(algorithm) +
              "</button>"
          );
        }
      );
      ...
      $("[name=crowd-tools-layout-algorithm-" + self.id + "]").on(
        "click",
        function () {
    ...
    self.tools.model.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-edit-dropdown-' + self.id + '"]').append(
        '<li class="dropdown-divider"></li> \
        <li class="dropdown"> \
          <button class="dropdown-item dropdown-toggle" type="button" id="crowd-tools-model-dropdown-' +
          self.id +
          '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            <i class="fa fa-fw fa-th"></i> Model \
          </button> \
          <ul class="dropdown-menu" aria-labelledby="crowd-tools-model-dropdown-' +
          self.id +
          '"></ul> \
        </li>'
      );
      ...
      $.each(
        ...
        function (conceptualModelName, conceptualModel) {
          if (conceptualModel.initPalette != null) {
            $(
              '[aria-labelledby="crowd-tools-model-dropdown-' + self.id + '"]'
            ).append(
              ...
              '<button class="dropdown-item" data-model="' +
                conceptualModelName +
                '" name="crowd-tools-model-advertisement-proceed-' +
                self.id +
                '" ' +
                (self.config.conceptualModel.name == conceptualModel.name
                  ? "disabled"
                  : "") +
                ">" +
                (self.config.conceptualModel.name == conceptualModel.name
      ...
      $("body").append(
        '<div id="crowd-tools-model-advertisement-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog"> \
            <div class="modal-content"> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> \
                <button name="crowd-tools-model-advertisement-proceed-' +
          self.id +
          '" \
                type="button" class="btn btn-danger" data-dismiss="modal">Proceed</button> \
              </div> \
      ...
      $("[name=crowd-tools-model-input-" + self.id + "]").on(
        "click",
        function () {
          $("#crowd-tools-model-advertisement-" + self.id).modal("show");
          $("#crowd-tools-model-advertisement-proceed-" + self.id).attr(
            "data-model",
      ...
      $("[name=crowd-tools-model-advertisement-proceed-" + self.id + "]").on(
        "click",
        function () {
          console.log("/editor/" + $(this).attr("data-model"));
          if (self.config.ngRouter)
            self.config.ngRouter.navigate(
              ["/editor/" + $(this).attr("data-model")],
              { state: { file: self.config.actualFile } }
            );
          else window.location.href = "/editor/" + $(this).attr("data-model");
        }
      );
    };
    ...
    self.tools.translate.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-edit-dropdown-' + self.id + '"]').append(
        '<li class="dropdown"> \
          <button class="dropdown-item dropdown-toggle" type="button" id="crowd-tools-translate-dropdown-' +
          self.id +
          '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            <i class="fa fa-fw fa-random"></i> Translate \
          </button> \
          <ul class="dropdown-menu" aria-labelledby="crowd-tools-translate-dropdown-' +
          self.id +
          '"></ul> \
        </li>'
      );
      ...
      $.each(
        ...
        function (conceptualModelName, conceptualModel) {
          if (conceptualModel.initPalette != null) {
            $(
              '[aria-labelledby="crowd-tools-translate-dropdown-' +
                self.id +
                '"]'
            ).append(
              '<button class="dropdown-item" data-model="' +
                conceptualModelName +
                '" name="crowd-tools-translate-schema-' +
                self.id +
                '" ' +
                (self.config.conceptualModel.name == conceptualModel.name
                  ? "disabled"
      ...
      $("[name=crowd-tools-translate-schema-" + self.id + "]").on(
        "click",
        function () {
          var btn = this;
          event.stopPropagation();
          ...
          var model = $(this).attr("data-model");
          $("#crowd-tools-translate-schema-" + self.id).attr(
            "data-model",
            model
          );
    ...
    self.tools.clearWorkspace.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-edit-dropdown-' + self.id + '"]').append(
        ...
        <li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="Clear Paper"> \
            <button class="dropdown-item" style="color: #dc3545;" id="crowd-tools-clear-workspace-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-clear-workspace-advertisement-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog"> \
            <div class="modal-content"> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> \
                <button id="crowd-tools-clear-workspace-advertisement-proceed-' +
          self.id +
          '" \
                type="button" class="btn btn-danger" data-dismiss="modal">Proceed</button> \
              </div> \
      ...
      $("#crowd-tools-clear-workspace-input-" + self.id).on(
        "click",
        function () {
          if (self.hasChanges()) {
            $("#crowd-tools-clear-workspace-advertisement-" + self.id).modal(
              "show"
            );
          }
          $(".tooltip").tooltip("hide");
      ...
      $("#crowd-tools-clear-workspace-advertisement-proceed-" + self.id).on(
        "click",
        function () {
  ...
  self.tools.view.init = function () {
    ...
    $("#crowd-tools-menu-" + self.id).append(
      '<div class="btn-group dropdown" id="crowd-tools-view-btn-' +
        self.id +
        '"> \
        <button class="btn btn-adaptive dropdown-toggle" type="button" id="crowd-tools-view-dropdown-' +
        self.id +
        '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
          <i class="fa fa-fw fa-eye"></i> View \
        </button> \
        <ul class="dropdown-menu" aria-labelledby="crowd-tools-view-dropdown-' +
        self.id +
        '"></ul> \
      </div>'
    );
    ...
    self.tools.viewScrollbar.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-view-dropdown-' + self.id + '"]').append(
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="Show/Hide Scrollbar"> \
            <button class="dropdown-item" id="crowd-tools-view-scrollbar-input-' +
          self.id +
          '"> \
      ...
      $("#crowd-tools-view-scrollbar-input-" + self.id).on(
        "click",
        function () {
          $("#crowd-workspace-" + self.id).css(
            "overflow",
            $("#crowd-workspace-" + self.id).css("overflow") == "hidden"
              ? "scroll"
              : "hidden"
          );
          $(".tooltip").tooltip("hide");
      ...
      $('[aria-labelledby="crowd-tools-view-dropdown-' + self.id + '"]').append(
        ...
        <li> \
          ...
          (self.config.conceptualModel.name != "uml"
            ? "This functionality is only available for UML model"
            : "Show/Hide Attributes and Methods on Classes") +
          '"> \
            <button class="dropdown-item" id="crowd-tools-collapse-classes-input-' +
          self.id +
          '" \
            ' +
          (self.config.conceptualModel.name != "uml"
            ...
            : "") +
          '> \
            <i id="crowd-tools-collapse-classes-icon-' +
          self.id +
          '" class="fa fa-fw fa-compress"></i> Collapse Classes</button> \
          </span> \
        </li>'
      );

      //event handler when click collapse classes
      $("#crowd-tools-collapse-classes-input-" + self.id).on(
        "click",
        function () {
          self.tools.collapseClasses.collapsed =
            !self.tools.collapseClasses.collapsed;
          $("#crowd-tools-collapse-classes-icon-" + self.id).removeClass(
            self.tools.collapseClasses.collapsed ? "fa-compress" : "fa-expand"
          );
          $("#crowd-tools-collapse-classes-icon-" + self.id).addClass(
            self.tools.collapseClasses.collapsed ? "fa-expand" : "fa-compress"
          );
          self.tools.collapseClasses.updateCollapsed();
    ...
    self.tools.toggleRolesAssoc.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-view-dropdown-' + self.id + '"]').append(
        '<li class="dropdown"> \
          ...
          ' +
          (self.config.conceptualModel.name != "uml"
            ? 'data-toggle="tooltip" data-placement="right" title="This functionality is only available for UML model"'
            : "") +
          '> \
          ...
          (self.config.conceptualModel.name != "uml"
            ...
            : "dropdown-toggle") +
          '" \
              type="button" id="crowd-tools-toggle-roles-assoc-dropdown-' +
          self.id +
          '" \
              data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
            ...
            ' +
          (self.config.conceptualModel.name == "uml"
            ? '<ul class="dropdown-menu" aria-labelledby="crowd-tools-toggle-roles-assoc-dropdown-' +
              self.id +
              '" \
            style="width: 220px"> \
              <li><button class="dropdown-item" id="crowd-tools-toggle-roles-assoc-input-roles-' +
              self.id +
              '"> \
              Toggle Roles \
              <i class="fa fa-fw fa-toggle-off pull-right" id="crowd-tools-toggle-roles-assoc-check-roles-' +
              self.id +
              '"></i> \
              </button> \
              </li> \
              <li><button class="dropdown-item" id="crowd-tools-toggle-roles-assoc-input-assoc-' +
              self.id +
              '"> \
              Toggle Associations \
              <i class="fa fa-fw fa-toggle-on pull-right" id="crowd-tools-toggle-roles-assoc-check-assoc-' +
              self.id +
              '"></i> \
              </button> \
              </li> \
      ...
      $("#crowd-tools-toggle-roles-assoc-input-roles-" + self.id).on(
        "click",
        function () {
          self.tools.toggleRolesAssoc.toggleRoles =
            !self.tools.toggleRolesAssoc.toggleRoles;
          $(
            "#crowd-tools-toggle-roles-assoc-check-roles-" + self.id
      ...
      $("#crowd-tools-toggle-roles-assoc-input-assoc-" + self.id).on(
        "click",
        function () {
          self.tools.toggleRolesAssoc.toggleAssoc =
            !self.tools.toggleRolesAssoc.toggleAssoc;
          $(
            "#crowd-tools-toggle-roles-assoc-check-assoc-" + self.id
          ).toggleClass("fa-toggle-on");
          $(
            "#crowd-tools-toggle-roles-assoc-check-assoc-" + self.id
          ).toggleClass("fa-toggle-off");
      ...
      self.tools.filterImplicitTerms.updateFilter = function () {
        ...
        self.workspace.graph.getElements().forEach(function (element) {
          if (
            ...
            element.prop("uri") == "http://www.w3.org/2002/07/owl#Nothing"
          ) {
            element.attr(
              "./display",
              self.tools.filterImplicitTerms.filtered ? "none" : true
            );
      ...
      $('[aria-labelledby="crowd-tools-view-dropdown-' + self.id + '"]').append(
        ...
        <li> \
          ...
          title="Filter implicit terms like owl:Thing and owl:Nothing from ontology"> \
            <button class="dropdown-item" id="crowd-tools-filter-implicit-terms-input-' +
          self.id +
          '"> \
            <i id="crowd-tools-filter-implicit-terms-icon-' +
          self.id +
          '" class="fa fa-fw fa-toggle-on"></i> Filter Implicit Terms</button> \
          </span> \
        </li>'
      );

      //event handler when filter implicit terms
      $("#crowd-tools-filter-implicit-terms-input-" + self.id).on(
        "click",
        function () {
          self.tools.filterImplicitTerms.filtered =
            !self.tools.filterImplicitTerms.filtered;
          $("#crowd-tools-filter-implicit-terms-icon-" + self.id).removeClass(
            self.tools.filterImplicitTerms.filtered
              ? "fa-toggle-off"
              : "fa-toggle-on"
          );
          $("#crowd-tools-filter-implicit-terms-icon-" + self.id).addClass(
            self.tools.filterImplicitTerms.filtered
              ? "fa-toggle-on"
              : "fa-toggle-off"
          );
  ...
  self.tools.tools.init = function () {
    ...
    $("#crowd-tools-menu-" + self.id).append(
      '<div class="btn-group dropdown" id="crowd-tools-tools-btn-' +
        self.id +
        '"> \
        <button class="btn btn-adaptive dropdown-toggle" type="button" id="crowd-tools-tools-dropdown-' +
        self.id +
        '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
          <i class="fa fa-fw fa-wrench"></i> Tools \
        </button> \
        <ul class="dropdown-menu" aria-labelledby="crowd-tools-tools-dropdown-' +
        self.id +
        '"></ul> \
      </div>'
    );
    ...
    self.tools.namespaces.init = function () {
      ...
      $(
        '[aria-labelledby="crowd-tools-tools-dropdown-' + self.id + '"]'
      ).append(
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" \
          title="' +
          (!self.config?.ngFiles?.user
            ? loggedInMessage
            : "Edit your collection of namespaces for use it on URIs") +
          '" > \
            <button class="dropdown-item" id="crowd-tools-namespaces-input-' +
          self.id +
          '" \
            ' +
          (!self.config?.ngFiles?.user
    ...
    self.tools.reasoning.init = function () {
            ...
            self.config.reasoningApi.request({
              ...
              success: (res) => {
                ...
                self.config.metamodelApi.request({
                  ...
                  success: (schema) => {
                    if (options?.success)
                      options?.success({ schema: schema, reasoning: res });
                  },
                  error: (error) => {
      ...
      $(
        ...
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="Call Reasoner"> \
            <button class="dropdown-item" id="crowd-tools-reasoning-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-reasoning-options-modal-' +
          self.id +
          '" class="modal fade"> \
            ...
            <div class="modal-content"> \
              ...
              <div class="modal-body"> \
                <div class="form-group"> \
                  ...
                  <select class="form-control custom-select my-1 mr-sm-2" id="crowd-tools-reasoning-options-reasoner-' +
          self.id +
          '"> \
                    <option>Racer</option> \
                ...
                <div class="form-group"> \
                  ...
                  <select class="form-control custom-select my-1 mr-sm-2" id="crowd-tools-reasoning-options-encoding-' +
          self.id +
          '"> \
                    <option>ALCQI</option> \
                  </select> \
                ...
                <div class="form-group form-check"> \
                  <input type="checkbox" class="form-check-input" id="crowd-tools-reasoning-options-cards-' +
          self.id +
          '"> \
                  <label class="form-check-label" for="crowd-tools-reasoning-options-cards-' +
          self.id +
          '">Reason Cardinalities</label><br> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button> \
                <button class="btn btn-warning" data-model="" id="crowd-tools-reasoning-call-' +
          self.id +
          '">Call Reasoner</button> \
              </div> \
            </div> \
          </div> \
        </div>'
      ...
      $("#crowd-tools-reasoning-call-" + self.id).on("click", function () {
        self.tools.reasoning.callReasoning({
          reasoner: $(
            "#crowd-tools-reasoning-options-reasoner-" +
              self.id +
              " option:selected"
          ).val(),
          strategy: $(
            "#crowd-tools-reasoning-options-encoding-" +
              self.id +
              " option:selected"
          )
            .val()
            .toLowerCase(),
          cards: $("#crowd-tools-reasoning-options-cards-" + self.id).is(
            ":checked"
          ),
          success: function (response) {
    ...
    self.tools.undoReasoning.init = function () {
      ...
      <span class="d-block" data-toggle="tooltip" data-placement="right" title="Remove Reasoning Inferences and Clear Marks"> \
        <button class="dropdown-item" style="color: #dc3545;" id="crowd-tools-undo-reasoning-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-undo-reasoning-advertisement-' +
          self.id +
          '" class="modal fade"> \
      <div class="modal-dialog"> \
        <div class="modal-content"> \
          ...
          <div class="modal-footer"> \
            <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> \
            <button id="crowd-tools-undo-reasoning-advertisement-proceed-' +
          self.id +
          '" \
            type="button" class="btn btn-danger" data-dismiss="modal">Proceed</button> \
          </div> \
      ...
      $("#crowd-tools-undo-reasoning-input-" + self.id).on(
        "click",
        function () {
          if (self.hasChanges()) {
            $("#crowd-tools-undo-reasoning-advertisement-" + self.id).modal(
              "show"
            );
          }
          $(".tooltip").tooltip("hide");
      ...
      $("#crowd-tools-undo-reasoning-advertisement-proceed-" + self.id).on(
        "click",
        function () {
    ...
    self.tools.clearReasoning.init = function () {
      ...
      $(
        ...
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="Preserve Reasoning Inferences and Clear Marks"> \
            <button class="dropdown-item" style="color: #dc3545;" id="crowd-tools-clear-reasoning-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-clear-reasoning-advertisement-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog"> \
            <div class="modal-content"> \
              ...
              <div class="modal-footer"> \
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> \
                <button id="crowd-tools-clear-reasoning-advertisement-proceed-' +
          self.id +
          '" \
                type="button" class="btn btn-danger" data-dismiss="modal">Proceed</button> \
              </div> \
      ...
      $("#crowd-tools-clear-reasoning-input-" + self.id).on(
        "click",
        function () {
          if (self.hasChanges()) {
            $("#crowd-tools-clear-reasoning-advertisement-" + self.id).modal(
              "show"
            );
          }
          $(".tooltip").tooltip("hide");
      ...
      $("#crowd-tools-clear-reasoning-advertisement-proceed-" + self.id).on(
        "click",
        function () {
    ...
    self.tools.repair.init = function () {
      ...
      self.tools.repair.openExplanationsWindow = function (options) {
        ...
        if (options?.Explanations) {
          Object.keys(options?.Explanations).forEach(function (key, index) {
            ...
            Object.keys(explanationsSet).forEach(function (key2, index2) {
              ...
              axiomsRadios +=
                ...
                (index2 + 1) +
                '"> \
                  <input class="form-check-input" type="radio" name="crowd-tools-repair-explanations-set-' +
                self.id +
                "-" +
                (index + 1) +
                '" \
                  id="crowd-tools-repair-explanations-set-' +
                self.id +
                "-" +
                (index + 1) +
                "-" +
                ...
                key2 +
                '"> \
                  <label class="form-check-label" for="crowd-tools-repair-explanations-set-' +
                self.id +
                "-" +
                (index + 1) +
                "-" +
            ...
            $(".crowd-tools-repair-explanations-sets").append(
              '<div id="crowd-tools-repair-explanations-set-' +
                self.id +
                "-" +
                (index + 1) +
                '" \
          ...
          $(
            'input[name^="crowd-tools-repair-explanations-set-' +
              self.id +
              '-"]'
          ).on("change", function () {
            //if propagate explanation is checked then check the same axiom in all explanations sets that has it
            if (
              $("#crowd-tools-repair-explanations-propagate-" + self.id).is(
                ":checked"
              )
            ) {
              var selectedAxiom =
                options?.Explanations[$(this).attr("data-explanation")][
                  $(this).attr("data-axiom")
                ];
              $(
                'input[name^="crowd-tools-repair-explanations-set-' +
                  self.id +
      ...
      $(
        ...
        <li> \
            <span class="d-block" data-toggle="tooltip" data-placement="right" title="Use the KF Repair Tool for Generate Explanations"> \
              <button class="dropdown-item" id="crowd-tools-repair-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-repair-options-modal-' +
          self.id +
          '" class="modal fade"> \
              <div class="modal-dialog modal-dialog-scrollable modal-sm"> \
                <div class="modal-content"> \
                  ...
                  <div class="modal-body"> \
                    <div class="form-group text-center"> \
                      <button class="btn btn-sm btn-danger" id="crowd-tools-repair-options-find-unsat-concepts-' +
          self.id +
          '" type="button" \
                    ...
                    <div class="form-group"> \
                      ...
                      <select class="form-control custom-select my-1 mr-sm-2" id="crowd-tools-repair-options-concept-' +
          self.id +
          '" disabled> \
                      </select> \
                    </div> \
                    <div class="form-group"> \
                      <label for="">Reasoner</label> \
                      <select class="form-control custom-select my-1 mr-sm-2" id="crowd-tools-repair-options-reasoner-' +
          self.id +
          '"> \
                        <option>JFact</option> \
                    ...
                    <div class="form-group row"> \
                      ...
                      <div class="col"> \
                        <input type="text" value="5" class="form-control" id="crowd-tools-repair-options-maxexplain-' +
          self.id +
          '"> \
                      </div> \
                    </div> \
                  ...
                  <div class="modal-footer"> \
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button> \
                    <button class="btn btn-warning" data-model="" id="crowd-tools-repair-call-' +
          self.id +
          '" disabled>Call Repair</button> \
                  </div> \
                </div> \
              </div> \
            </div>'
      ...
      $("#crowd-workspace-" + self.id).append(
        '<div class="crowd-tools-repair-explanations-container d-none"> \
          ...
          <div> \
            ...
            <div class="col ml-1"> \
              <input type="checkbox" class="form-check-input" id="crowd-tools-repair-explanations-propagate-' +
          self.id +
          '" checked> \
              <label class="form-check-label" for="crowd-tools-repair-explanations-propagate-' +
          self.id +
          '"><b>Propagate Selection</b></label> \
            </div> \
            <div class="crowd-tools-repair-explanations-sets mt-2"> \
            </div> \
            <div class="text-center mt-2"> \
              <button class="btn btn-default text-secondary" id="crowd-tools-repair-explanations-cancel-' +
          self.id +
          '">Cancel</button> \
              <button class="btn btn-warning" id="crowd-tools-repair-explanations-apply-' +
          self.id +
          '">Apply</button> \
            </div> \
          </div> \
      ...
      $("#crowd-tools-repair-input-" + self.id).on("click", function () {
        ...
        $("#crowd-tools-repair-options-concept-" + self.id).prop(
          "disabled",
          true
        );
      ...
      $("#crowd-tools-repair-options-find-unsat-concepts-" + self.id).on(
        "click",
        function () {
          //disable find unsat concepts button
          $("#crowd-tools-repair-options-find-unsat-concepts-" + self.id).prop(
            "disabled",
            true
          );
          self.tools.reasoning.callReasoning({
            ...
            success: function (response) {
              ...
              response.reasoning["KF output"][
                "UNSATisfiable Entity types"
              ].forEach(function (concept) {
                $("#crowd-tools-repair-options-concept-" + self.id).append(
              ...
              if (
                response.reasoning["KF output"]["UNSATisfiable Entity types"]
                  .length > 0
              ) {
                $("#crowd-tools-repair-options-concept-" + self.id).prop(
                  "disabled",
                ...
                $("#crowd-tools-repair-call-" + self.id).prop(
                  "disabled",
                  false
                );
              } else {
              ...
              $(
                "#crowd-tools-repair-options-find-unsat-concepts-" + self.id
              ).prop("disabled", false);
            },
            error: function (error) {
              //enable find unsat concepts button
      ...
      $("#crowd-tools-repair-call-" + self.id).on("click", function () {
        ...
        self.tools.repair.callRepair({
          reasoner: $(
            "#crowd-tools-repair-options-reasoner-" +
              self.id +
              " option:selected"
          ).val(),
          entity: $(
            "#crowd-tools-repair-options-concept-" +
              self.id +
              " option:selected"
          ).val(),
          maxExplanations:
            $("#crowd-tools-repair-options-maxexplain-" + self.id).val() || 5,
          timeout: 120000,
          success: function (response) {
      ...
      $("#crowd-tools-repair-explanations-cancel-" + self.id).on(
        "click",
        function () {
      ...
      $("#crowd-tools-repair-explanations-apply-" + self.id).on(
        "click",
        function () {
          //hide explanations window
          $(".crowd-tools-repair-explanations-container").addClass("d-none");
  ...
  self.tools.help.init = function () {
    //append edit dropdown for all tools related tools
    $("#crowd-tools-menu-" + self.id).append(
      '<div class="btn-group dropdown" id="crowd-tools-help-btn-' +
        self.id +
        '"> \
        <button class="btn btn-adaptive dropdown-toggle" type="button" id="crowd-tools-help-dropdown-' +
        self.id +
        '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> \
          <i class="fa fa-fw fa-question"></i> Help \
        </button> \
        <ul class="dropdown-menu" aria-labelledby="crowd-tools-help-dropdown-' +
        self.id +
        '"></ul> \
      </div>'
    );
    ...
    self.tools.tutorials.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-help-dropdown-' + self.id + '"]').append(
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="Video Tutorials List"> \
            <button class="dropdown-item" id="crowd-tools-help-tutorials-input-' +
          self.id +
          '"> \
      ...
      $("#crowd-tools-help-tutorials-input-" + self.id).on(
        "click",
        function () {
    ...
    self.tools.about.init = function () {
      ...
      $('[aria-labelledby="crowd-tools-help-dropdown-' + self.id + '"]').append(
        '<li> \
          <span class="d-block" data-toggle="tooltip" data-placement="right" title="About crowd tool and team"> \
            <button class="dropdown-item" id="crowd-tools-help-about-input-' +
          self.id +
          '"> \
      ...
      $("body").append(
        '<div id="crowd-tools-help-about-modal-' +
          self.id +
          '" class="modal fade"> \
          <div class="modal-dialog modal-dialog-scrollable modal-sm"> \
            <div class="modal-content"> \
  ...
  self.tools.zoom.init = function () {
    ...
    $("#crowd-tools-row-" + self.id).append(
      '<div class="form-group"> \
        ...
        <label id="crowd-tools-zoom-label-' +
        self.id +
        '" style="float: right">100%</label> \
        <input class="form-control-range" id="crowd-tools-zoom-input-' +
        self.id +
        '" type="range" min="25" max="250" step="25" value="100" /> \
      </div>'
    );
  ...
  self.tools.gridSize.init = function () {
    ...
    $("#crowd-tools-row-" + self.id).append(
      '<div class="form-group"> \
        ...
        <label id="crowd-tools-grid-size-label-' +
        self.id +
        '" style="float: right">10</label> \
        <input class="form-control-range" id="crowd-tools-grid-size-input-' +
        self.id +
  ...
  self.tools.fullscreen.init = function () {
    ...
    $("#crowd-tools-row-" + self.id).append(
      '<div class="form-group"> \
        <button class="btn btn-primary" id="crowd-tools-fullscreen-input-' +
        self.id +
        '" type="button" \
...
CrowdEditor.prototype.initWorkspace = function () {
  ...
  $("#crowd-workspace-" + self.id).append(
    '<div class="crowd-workspace-scrollable" id="crowd-workspace-scrollable-' +
      self.id +
      '"></div>'
  );

  //append dom element for workspace paper
  $("#crowd-workspace-scrollable-" + self.id).append(
    '<div class="crowd-workspace-paper" id="crowd-workspace-paper-' +
      self.id +
      '"></div>'
  );
  ...
  self.workspace.renderElementTools = function (elementView) {
    if (
      elementView.model.attributes.enableTools == null ||
      elementView.model.attributes.enableTools
    ) {
      self.workspace.paper.hideTools();
      var toolsView =
        self.workspace.tools.elements.elementsToolsView[
          elementView.model.attributes.type
        ];
      elementView.addTools(
        toolsView != null
          ? toolsView
          : self.workspace.tools.elements.elementsToolsView.basic
      );
  ...
  self.workspace.centerScroll = function () {
    ...
    var heightToFit =
      ...
      $("#crowd-workspace-paper-" + self.id).height()
        ? $("#crowd-workspace-" + self.id).height()
        : $("#crowd-workspace-paper-" + self.id).height();
    $("#crowd-workspace-" + self.id).scrollLeft(
      ($("#crowd-workspace-scrollable-" + self.id).width() - widthToFit) / 2 -
        offsetWidth
    );
    $("#crowd-workspace-" + self.id).scrollTop(
      ($("#crowd-workspace-scrollable-" + self.id).height() - heightToFit) / 2 -
        offsetHeight
    );
  };
  ...
  self.workspace.fitPaper = function () {
    ...
    if ($("#crowd-workspace-paper-" + self.id).position()) {
      var prop = {
        left:
          ($("#crowd-workspace-" + self.id).scrollLeft() -
            $("#crowd-workspace-paper-" + self.id).position().left) /
          $("#crowd-workspace-paper-" + self.id).width(),
        top:
          ($("#crowd-workspace-" + self.id).scrollTop() -
            $("#crowd-workspace-paper-" + self.id).position().top) /
          $("#crowd-workspace-paper-" + self.id).height(),
        width:
          $("#crowd-workspace-" + self.id).width() -
          $("#crowd-workspace-paper-" + self.id).width(),
    ...
    self.workspace.paper.fitToContent({
      ...
      contentArea: {
        x:
          self.workspace.paper.getContentArea().x > 0
            ? 0
            : self.workspace.paper.getContentArea().x,
        y:
          self.workspace.paper.getContentArea().y > 0
            ? 0
            : self.workspace.paper.getContentArea().y,
        width:
          self.workspace.paper.getContentArea().x > 0
            ? Math.abs(self.workspace.paper.getContentArea().x) +
              self.workspace.paper.getContentArea().width
            : self.workspace.paper.getContentArea().width,
        height:
          self.workspace.paper.getContentArea().y > 0
            ? Math.abs(self.workspace.paper.getContentArea().y) +
    ...
    $("#crowd-workspace-scrollable-" + self.id).width(
      $("#crowd-workspace-scrollable-" + self.id).width() +
        paperAreaDiff.width * self.workspace.zoom
    );
    $("#crowd-workspace-scrollable-" + self.id).height(
      $("#crowd-workspace-scrollable-" + self.id).height() +
        paperAreaDiff.height * self.workspace.zoom
    );
    ...
    if (zoomChange) {
      ...
      $("#crowd-workspace-" + self.id).scrollLeft(
        $("#crowd-workspace-paper-" + self.id).width() * prop.left +
          $("#crowd-workspace-paper-" + self.id).position().left +
          Math.abs(newProp.width - prop.width) *
            (self.workspace.zoom - zoomPrev)
      );
      $("#crowd-workspace-" + self.id).scrollTop(
        $("#crowd-workspace-paper-" + self.id).height() * prop.top +
          $("#crowd-workspace-paper-" + self.id).position().top +
          Math.abs(newProp.height - prop.height) *
            (self.workspace.zoom - zoomPrev)
      );
    }
  ...
  self.workspace.graph.on(
    "change:position change:target change:source remove add",
    function () {
  ...
  $("#crowd-workspace-scrollable-" + self.id).on(
    "mouseup mouseleave",
    function (evt) {
      self.workspace.dragStartPosition = null;
    }
  ...
  $("#crowd-workspace-scrollable-" + self.id).on("mousemove", function (evt) {
    if (self.workspace.dragStartPosition) {
      ...
      //scroll the element
      $("#crowd-workspace-" + self.id)[0].scrollTop =
        self.workspace.dragStartPosition.top - dy;
      $("#crowd-workspace-" + self.id)[0].scrollLeft =
        self.workspace.dragStartPosition.left - dx;
    }
  });
};
...
CrowdEditor.prototype.initElementsToolsViews = function () {
  ...
  self.workspace.tools.elements.markup = function (config) {
    return [
      //draw a circle for the icon background
      {
        ...
        style: {
          opacity:
            config.attributes?.circle?.opacity != null
              ? config.attributes?.circle?.opacity
              : "0.7",
        },
        attributes: {
          r:
            config.attributes?.circle?.r != null
              ? config.attributes?.circle?.r
              : 10,
          cx:
            config.attributes?.circle?.cx != null
              ? config.attributes?.circle?.cx
              : 0,
          cy:
            config.attributes?.circle?.cy != null
              ? config.attributes?.circle?.cy
              : -10,
          fill: config.background != null ? config.background : "black",
          cursor:
            config.attributes?.circle?.cursor != null
              ? config.attributes?.circle?.cursor
              : "pointer",
        },
      },
      ...
      //https://material.io/resources/icons
      {
        ...
        style: {
          "font-size": config.attributes?.text?.fontSize
            ? config.attributes?.text?.fontSize
            : "14px",
        },
        attributes: {
          x:
            config.attributes?.text?.x != null
              ? config.attributes?.text?.x
              : -7,
          y:
            config.attributes?.text?.y != null
              ? config.attributes?.text?.y
              : -3,
          fill:
            config.attributes?.text?.color != null
              ? config.attributes?.text?.color
              : "white",
          cursor:
            config.attributes?.text?.cursor != null
              ? config.attributes?.text?.cursor
              : "pointer",
          title: config.tooltip?.title != null ? config.tooltip?.title : "",
          "data-toggle": "tooltip",
          "data-placement":
            config.tooltip?.placement != null
              ? config.tooltip?.placement
              : "top",
        },
  ...
  self.workspace.tools.elements.removeTool = new joint.elementTools.Remove({
    ...
    rotate: true,
    offset: { x: -25, y: -15 },
    markup: self.workspace.tools.elements.markup({
      icon: "clear",
      tooltip: { title: "Click to remove the object", placement: "left" },
    }),
  });
  ...
  self.workspace.tools.elements.linkTool = function (config) {
    return new joint.elementTools.Button({
      ...
      action: function (evt, elementView, buttonView) {
        ...
        //create the link
        var link =
          config.link && config.link.type
            ? self.palette.links[config.link.type].clone()
            : self.palette.links.basic.clone();
        ...
        link.target({
          x:
            (eventPoint.x - self.workspace.paper.translate().tx) /
            self.workspace.paper.scale().sx,
          y:
            (eventPoint.y - self.workspace.paper.translate().ty) /
            self.workspace.paper.scale().sy,
        ...
        if (self.config.enumerate) {
          link.prop(
            "uri",
            link.prop("uri") + "-" + self.enumerate.getNumber(link)
          );
        }
        ...
        clickEvent.initMouseEvent(
          ...
          null,
          null,
          new EventTarget("marker-arrowhead")
        );
      ...
      markup:
        config.markup != null
          ...
          : self.workspace.tools.elements.markup({
              icon: "call_made",
              tooltip: {
                title: "Click and drag to connect the object",
                placement: "right",
              },
            }),
    });
  ...
  self.workspace.tools.elements.cloneTool = new joint.elementTools.Button({
    ...
    action: function (evt, elementView, buttonView) {
      ...
      clonedElement.attributes.attrs = $.extend(
        true,
        {},
      ...
      clonedElement.attributes.position = {
        x:
          (eventPoint.x - self.workspace.paper.translate().tx) /
            self.workspace.paper.scale().sx -
          clonedElement.attributes.size.width / 2,
        y:
          (eventPoint.y - self.workspace.paper.translate().ty) /
            self.workspace.paper.scale().sy -
          clonedElement.attributes.size.height / 2,
    ...
    markup: self.workspace.tools.elements.markup({
      icon: "content_copy",
      tooltip: {
        title: "Click and drag to clone the object",
        placement: "right",
      },
    }),
  });
  ...
  self.workspace.tools.elements.removeLinksTool = new joint.elementTools.Button(
    {
      ...
      action: function (evt, elementView, buttonView) {
        ...
        self.workspace.graph
          ...
          .forEach(function (connectedLink) {
      ...
      markup: self.workspace.tools.elements.markup({
        icon: "content_cut",
        tooltip: {
          title: "Click to break all connections to other objects",
          placement: "left",
        },
      }),
    }
    ...
    return new joint.elementTools.Button({
      ...
      action: function (evt, elementView, buttonView) {
        ...
        if (self.config.enumerate) {
          newElement.prop(
            "uri",
            newElement.prop("uri") + "-" + self.enumerate.getNumber(newElement)
          );
        }
      ...
      markup:
        config.markup != null
          ...
          : self.workspace.tools.elements.markup({
              icon: "share",
              tooltip: {
                title: "Click and drag to make new object and connect with it",
                placement: "top",
              },
            }),
    });
  ...
  self.workspace.tools.elements.resizeTool = function (config) {
    return new joint.elementTools.Button({
      ...
      action: function (evt, elementView, buttonView) {
        ...
        $("#crowd-workspace-" + self.id).on(
          ...
          function (evt) {
            ...
            var newSize = {
              width: elementView.model.size().width,
              height: elementView.model.size().height,
            };
            newSize.width =
              newSize.width + addSize.width > config.minSize.width &&
              newSize.width + addSize.width < config.maxSize.width
                ? newSize.width + addSize.width
                : newSize.width;
            newSize.height =
              newSize.height + addSize.height > config.minSize.height &&
              newSize.height + addSize.height < config.maxSize.height
                ? newSize.height + addSize.height
                : newSize.height;
            ...
            if (config.snapGrid || config.snapGrid == null) {
              ...
              var gridSize = self.workspace.paper.options.gridSize;
              newSize.width =
                Math.round(newSize.width / gridSize) * gridSize >
                ...
                Math.round(newSize.width / gridSize) * gridSize <
                  ...
                  : newSize.width;
              newSize.height =
                Math.round(newSize.height / gridSize) * gridSize >
            ...
            elementView.model.resize(newSize.width, newSize.height, {
              direction: config.direction,
            });

            //re-set name for wrap it to the new size
            if (elementView.model.prop("name"))
              elementView.model.trigger(
                "change:name",
                elementView.model,
                elementView.model.prop("name")
              );
          }
        );
      ...
      markup:
        config.markup != null
          ? config.markup
          : self.workspace.tools.elements.resizeMarkup({}),
    });
  };
  ...
  self.workspace.tools.elements.basicTools = [
    ...
    self.workspace.tools.elements.resizeTool({
      ...
      vdirection: "top",
      hdirection: "left",
      markup: self.workspace.tools.elements.resizeMarkup({
        cursor: "nw-resize",
      }),
    }),
    self.workspace.tools.elements.resizeTool({
...
CrowdEditor.prototype.initLinksToolsViews = function () {
  ...
  self.workspace.tools.links.linkRolePort = function (config) {
    ...
    self.workspace.paper.on("render:done", function (evt, b, c, d, e) {
      if (parentView) {
        var labelPoint = parentView.getLabelCoordinates(
          config.position ? config.position : { distance: 0.25, offset: 0 }
        );
        fakePort.position(
          labelPoint.x - fakePort.size().width / 2,
          labelPoint.y - fakePort.size().height / 2
        );
      }
    });
  };
  ...
  self.workspace.graph.on("add", function (link) {
    if (link.isLink()) {
      ...
      var linkToolsView =
        self.workspace.tools.links.linksToolsView[
          linkView.model.attributes.type
        ];
      ...
      linkView.addTools(
        linkToolsView != null
          ? linkToolsView()
          : self.workspace.tools.links.linksToolsView.basic()
      );
...
CrowdEditor.prototype.initInspector = function () {
  ...
  self.inspector.toggleContent = function (toggle) {
    //show empty message
    $("#crowd-inspector-" + self.id + " .crowd-inspector-empty").toggle(
      !toggle
    );
    ...
    //hide content
    $("#crowd-inspector-" + self.id + " .crowd-inspector-content").toggle(
      toggle
    );
  };
  ...
  self.inspector._makeAttributeDom = function (attribute) {
    ...
    switch (attribute.type) {
        ...
        dom = $(
          ...
          <div class="col"> \
            <div class="form-check"> \
              <input class="form-check-input" type="checkbox" value="" id="crowd-inspector-content-' +
            attribute.elementID +
            "-" +
            self.id +
            '"> \
              <label class="form-check-label" for="crowd-inspector-content-' +
            attribute.elementID +
            "-" +
            self.id +
            '">' +
        ...
        attribute.values.forEach(function (value) {
          $(dom)
            ...
            .append(
              ...
              <input class="form-check-input" type="radio" name="crowd-inspector-content-' +
                attribute.elementID +
                "-" +
                self.id +
                '" \
              id="crowd-inspector-content-' +
                attribute.elementID +
                "-" +
                value.value +
                "-" +
        ...
        dom = $(
          ...
          <div class="col"> \
            ...
            (attribute.label
              ...
              : "") +
            '\
              <select class="custom-select custom-select-sm" id="crowd-inspector-content-' +
            attribute.elementID +
            "-" +
            self.id +
            '"> \
      ...
      case "alert":
        dom = $(
          ...
          <div class="alert alert-' +
            (attribute.color ? attribute.color : "dark") +
            '" role="alert" \
          id="crowd-inspector-content-' +
            attribute.elementID +
            "-" +
      ...
      case "object":
        dom = $(
          '<span class="row"> \
          <div class="col"> \
            <div class="form-group" id="crowd-inspector-parameters-' +
            attribute.elementID +
            "-" +
            self.id +
            '"> \
        ...
        attribute.parameters.forEach(function (parameter) {
          ...
          parameter.index = "";
          parameter.elementID = formatSelector(
            parameter.container +
              "-" +
              parameter.property +
              "-" +
      ...
      case "list":
        dom = $(
          ...
          <div class="form-group"> \
            ...
            (attribute.label ? "<label>" + attribute.label + "</label>" : "") +
            '\
            <div class="form-group" id="crowd-inspector-content-elements-' +
            attribute.elementID +
            "-" +
            self.id +
            '"></div> \
            <div class="text-center"> \
              <button class="btn btn-primary" id="crowd-inspector-content-add-' +
            attribute.elementID +
            "-" +
            self.id +
            '" \
        ...
        break;
      case "uri":
        self.inspector.uriEvents.push(attribute.elementID);
        ...
        dom = $(
          '<span class="row"> \
            ...
            self.id +
            '"></span> \
                <select class="custom-select custom-select-sm" id="crowd-inspector-content-namespace-' +
            attribute.elementID +
            "-" +
            self.id +
            '" \
          ...
          $(dom)
            ...
            .append(
              '<option value="' +
                ...
                '" title="' +
                self.config.defaultNamespace +
                '" selected>crowd</option>'
            );
          crowdOperations.forEach(function (operation) {
            $(dom)
              .find("select")
              .append(
                '<option value="' +
      ...
      default:
        dom = $(
          ...
          <div class="col"> \
            ...
            '" \
              class="form-control" id="crowd-inspector-content-' +
            attribute.elementID +
            "-" +
            self.id +
            '" /> \
    ...
    if (attribute.canRemove) {
      $(dom).append(
        '<div class="col-2"> \
          <button class="btn btn-danger"  id="crowd-inspector-content-remove-' +
          attribute.elementID +
          "-" +
          self.id +
          '" \
    ...
    var jumpLine;
    if (attribute.container == "") jumpLine = $("<hr>");
  ...
  self.inspector.addAttribute = function (attribute) {
    ...
    attribute.container =
      attribute.container != null ? attribute.container : "";
    attribute.parentProperty =
      attribute.parentProperty != null ? attribute.parentProperty : "";
    attribute.index = attribute.index != null ? attribute.index : "";
    //compound the element ID for the actual attribute
    attribute.elementID = formatSelector(
      attribute.container + "-" + attribute.property + "-" + attribute.index
    );
    ...
    //store elementIDs for add events later
    self.inspector.uriEvents = [];

    //append dom element to the inspector content
    if (attribute.container != "")
      $(
        "#crowd-inspector-content-elements-" +
          formatSelector(attribute.container) +
          "-" +
          self.id
      ).append(self.inspector._makeAttributeDom(attribute));
    else
      $("#crowd-inspector-" + self.id + " .crowd-inspector-content").append(
        self.inspector._makeAttributeDom(attribute)
      );

    self.inspector.uriEvents.forEach(function (elementID) {
      // only allow write alphanumeric and - and _ characters on the fragment
      $("#crowd-inspector-content-fragment-" + elementID + "-" + self.id).on(
        "input",
        function () {
          var value = $(this).val();
    ...
    if (attribute.canRemove) {
      $(
        "#crowd-inspector-content-remove-" + attribute.elementID + "-" + self.id
      ).on("click", function () {
  ...
  self.inspector._setAttribute = function (attribute) {
    ...
    var propertyValue = attribute.map
      ? Object.keys(attribute.map).find(
          (key) =>
            attribute.map[key] ==
    ...
    switch (attribute.type) {
      case "boolean":
        ...
        ).prop("checked", propertyValue == "true");
        break;
      case "multiple":
        $(
          "#crowd-inspector-content-" +
            attribute.elementID +
            "-" +
            propertyValue +
            "-" +
      ...
      case "list":
        if (propertyValue) {
          ...
          $(
            ...
            //   canRemove: true
            // }));
            self.inspector.model.prop(
              attribute.property +
                "/" +
                self.inspector.model.prop(attribute.property).length,
  ...
  self.inspector._setEventAttribute = function (attribute) {
    ...
    $(
      ...
      self.inspector.model.prop(
        attribute.propertyPath,
        newPropertyValue == "null" ? null : newPropertyValue
      );
    });
  };
  ...
  self.inspector.loadContent = function () {
    if (self.inspector.model) {
      ...
      if (
        $(document.activeElement).prop("tagName") == "TEXTAREA" ||
      ...
      $("#crowd-inspector-" + self.id + " .crowd-inspector-title").html(
        self.inspector.model.attributes.label
          ? self.inspector.model.attributes.label
          : formatString(self.inspector.model.attributes.type)
      );
      ...
      if (
        self.inspector.lastFocus &&
        $("#" + self.inspector.lastFocus.elem)[0]
      ) {
        ...
        setSelectionRange(
          $("#" + self.inspector.lastFocus.elem)[0],
          self.inspector.lastFocus.caret.start,
          self.inspector.lastFocus.caret.end
        );
        //reset last focus
...
CrowdEditor.prototype.initMap = function () {
  ...
  $("#crowd-map-" + self.id).append(
    '<div class="crowd-map-scrollable" id="crowd-map-scrollable-' +
      self.id +
      '"></div>'
  );

  //append dom element for map paper
  $("#crowd-map-scrollable-" + self.id).append(
    '<div class="crowd-map-paper" id="crowd-map-paper-' + self.id + '"></div>'
  );

  //append dom element for camera view over workspace paper
  $("#crowd-map-scrollable-" + self.id).append(
    '<div class="crowd-map-camera" id="crowd-map-camera-' + self.id + '"></div>'
  );
  ...
  self.map.fitPaper = function () {
    ...
    var resizeRatio =
      scaledPaperSize.width >= scaledPaperSize.height
    ...
    self.map.paper.fitToContent({
      ...
      contentArea: {
        x:
          self.map.paper.getContentArea().x > 0
            ? 0
            : self.map.paper.getContentArea().x,
        y:
          self.map.paper.getContentArea().y > 0
            ? 0
            : self.map.paper.getContentArea().y,
        width:
          self.map.paper.getContentArea().x > 0
            ? self.map.paper.getContentArea().x +
    ...
    $("#crowd-map-paper-" + self.id).css({
      left:
        ($("#crowd-map-scrollable-" + self.id).width() -
          $("#crowd-map-paper-" + self.id).width()) /
        2,
      top:
        ($("#crowd-map-scrollable-" + self.id).height() -
          $("#crowd-map-paper-" + self.id).height()) /
        2,
    });
...
CrowdEditor.prototype.toJSONSchema = function () {
  ...
  return self.config.conceptualModel.toJSONSchema
    ? self.config.conceptualModel.toJSONSchema(self)
    : {};
};
...
CrowdEditor.prototype.fromJSONSchema = function (schema) {
  ...
  if (
    self.events.onAfterFromSchema &&
    Array.isArray(self.events.onAfterFromSchema)
  ) {
    //call all events registered
    self.events.onAfterFromSchema.forEach(function (event) {
      event();
    });
    //clear the events
    self.events.onAfterFromSchema = [];
...
CrowdEditor.prototype.initSyntaxValidator = function () {
  ...
  if (self.syntax.validateCell) {
    ...
    self.workspace.paper.on(
      "link:connect link:disconnect",
      function (linkView, evt, cellDisconnected) {
    ...
    self.workspace.graph.on(
      "add remove " +
        $.map(self.syntax.events, function (e) {
          return "change:" + e;
        }).join(" "),
      function (cell) {
        self.syntax.validateCell(cell);
        if (cell.isLink()) {
          var sourceCell = self.workspace.graph.getCell(
            cell.attributes.source.id
          );
          var targetCell = self.workspace.graph.getCell(
...
CrowdEditor.prototype.initReasoningValidator = function () {
  ...
  self.reasoning.undoSemantic = function () {
    self.workspace.graph.getCells().forEach(function (cell) {
      if (
        cell.prop("semantic/contents")?.some(function (e) {
          return e.value == "inferred" && e.inferredType != "cardinality";
        })
      ) {
        cell.remove();
      } else {
  ...
  self.reasoning.genericMark = function (reasoning) {
    ...
    var unsatCells = reasoning["KF output"][
      "UNSATisfiable Entity types"
    ].concat(reasoning["KF output"]["UNSATisfiable Roles"]);
    unsatCells.forEach(function (uri) {
      ...
      cell?.prop("semantic/contents/" + cell.prop("semantic/contents").length, {
        value: "unsatisfiable",
        text: '<span class="crowd-unsat-color"><b>Unsatisfiable</b></span>',
      });
    });
    ...
    subsumptionsCells.forEach(function (uri) {
      ...
      cell?.prop("semantic/contents/" + cell.prop("semantic/contents").length, {
        value: "inferred",
        text: '<span class="crowd-inferred-color"><b>Inferred Subsumption</b></span>',
      });
      if (cell?.isElement()) {
        self.workspace.graph.getConnectedLinks(cell).forEach(function (link) {
          link.prop(
            "semantic/contents/" + link.prop("semantic/contents").length,
            {
              value: "inferred",
              text: '<span class="crowd-inferred-color"><b>Inferred Subsumption</b></span>',
            }
    ...
    var cardinalitiesInferred =
      reasoning["KF output"]["Object types cardinalities"];
    cardinalitiesInferred.forEach(function (card) {
      ...
      cell?.prop("semantic/contents/" + cell.prop("semantic/contents").length, {
        inferredType: "cardinality",
        value: "inferred",
        text:
          '<span class="crowd-inferred-color"><b>Inferred Cardinality for role </b><i>' +
          kfRole.rolename +
          "</i></span>",
      });
    });
    ...
    $.each(reasoning["OWL Axioms"], function (axiom, axiomPairs) {
      axiomPairs.forEach(function (uris) {
        ...
        if (
          cells[0] &&
          cells[1] &&
          !unsatCells.find(function (cell) {
            return cell == cells[0].prop("uri") || cell == cells[1].prop("uri");
          })
        ) {
          cells[0]?.prop(
  ...
  self.reasoning.importPositionedSchema = function (schema) {
    ...
    var positionedSchema = self.config.conceptualModel.positioningJSONSchema(
      schema,
      self.toJSONSchema()
    );
...
CrowdEditor.prototype.initRepairingTools = function () {
  ...
  self.repairing.genericRepairMark = function (repair, selectedAxioms) {
    ...
    selectedAxioms.forEach(function (selectedAxiom) {
      ...
      var mappedEntity =
        repair["TBoxMap"][
          repair["Explanations"][selectedAxiom.explanation][selectedAxiom.axiom]
        ];

      self.repairing
        .findCellsByAxiom(repair, mappedEntity)
        .forEach(function (cell) {
          ...
          if (mappedEntity.split("@")[0] != "DisjointObjectType") {
            if (
              !cell.prop("repairing/contents")?.some(function (e) {
                return e.value == "keep";
              })
            ) {
              cell.prop("repairing", { title: "Repairing", contents: [] });
              cell.prop(
                "repairing/contents/" + cell.prop("repairing/contents").length,
                {
                  value: "remove",
                  text: '<span class="crowd-repairing-remove-color"><b>It will be removed if Repair is applied.</b></span>',
                }
              );
            }
          } else {
            ...
            cell.prop(
              ...
              {
                value: "restrict",
                text: '<span class="crowd-repairing-restrict-color"><b>It will be kept if Repair is applied but the Disjoint restriction were removed.</b></span>',
              }
            );
          }
    ...
    self.workspace.graph.getCells().forEach(function (cell) {
      if (
        !cell.prop("repairing/contents") ||
        cell.prop("repairing/contents").length == 0
      ) {
        cell.prop(
          "repairing/contents/" + cell.prop("repairing/contents").length,
          {
            value: "keep",
            text: '<span class="crowd-repairing-keep-color"><b>It will be kept if Repair is applied.</b></span>',
          }
        );
      }
    });
  ...
  self.repairing.findCellsByAxiom = function (repair, mappedAxiom) {
    ...
    switch (entityType) {
      case "Subsumption":
        entity = repair["KF"]["Relationship"]["Subsumption"].find(function (
          subsumption
        ) {
      ...
      case "DisjointObjectType":
        entity = repair["KF"]["Constraints"]["Disjointness constraints"][
          "Disjoint object type"
        ].find(function (disjointClasses) {
    ...
    switch (entityType) {
      case "Subsumption":
        ...
        cells[0] = self.workspace.graph
          .getConnectedLinks(childCell)
          .find(function (link) {
            return (
              link.prop("source").id == childCell.id &&
              link.prop("target").id == parentCell.id &&
              link.prop("type") == self.repairing.typeName(entityType + "Link")
            );
          });
        ...
        if (cells[0] == null) {
          ...
          self.workspace.graph
            ...
            .forEach(function (link) {
              if (
                link.prop("type") ==
                self.repairing.typeName(entityType + "Link")
              ) {
                var subsumptionElement =
                  link.prop("source").id == childCell.id
                    ? self.workspace.graph.getCell(link.prop("target").id)
                    : self.workspace.graph.getCell(link.prop("source").id);

                var connectedLinksSubsumptionElement =
                  self.workspace.graph.getConnectedLinks(subsumptionElement);
                connectedLinksSubsumptionElement.forEach(function (sublink) {
                  if (
                    ...
                    if (
                      (sublink.prop("source").id == subsumptionElement.id &&
                        sublink.prop("target").id == parentCell.id) ||
                      (sublink.prop("target").id == subsumptionElement.id &&
                        sublink.prop("source").id == parentCell.id)
                    ) {
      ...
      case "DisjointObjectType":
        ...
        entity.entities.forEach(function (entity) {
          disjointEntitiesCells.push(
            self.workspace.graph.getCells().find(function (cell) {
        ...
        self.workspace.graph
          .getConnectedLinks(disjointEntitiesCells[0])
          .forEach(function (link) {
            //check if the link connects with a disjoint element cell
            var disjointElementCell =
              link.prop("source").id == disjointEntitiesCells[0].id
                ? self.workspace.graph.getCell(link.prop("target").id)
                : self.workspace.graph.getCell(link.prop("source").id);

            if (
              disjointElementCell.prop("type") ==
              self.repairing.typeName(entityType + "Element")
            ) {
              ...
              for (var i = 1; i < disjointEntitiesCells.length; i++) {
                var cellConnectionToDisjoint = self.workspace.graph
                  .getConnectedLinks(disjointEntitiesCells[i])
                  .find(function (link) {
                    return (
                      (link.prop("source").id == disjointEntitiesCells[i].id &&
                        link.prop("target").id == disjointElementCell.id) ||
                      (link.prop("target").id == disjointEntitiesCells[i].id &&
                        link.prop("source").id == disjointElementCell.id)
                    );
                  });
                disjointElementCellConnected =
                  disjointElementCellConnected && cellConnectionToDisjoint;
  ...
  self.repairing.genericHighlightAxiom = function (repair, selectedAxiom) {
    ...
    self.workspace.graph.getCells().forEach(function (cell) {
      if (cell.prop("highlighted")) cell.prop("highlighted", false);
    });
...
CrowdEditor.prototype.toBase64 = function (options, callback) {
  ...
  var canvasContainer = options.miniature
    ? "#crowd-map-paper-" + self.id
    : "#crowd-workspace-paper-" + self.id;
  ...
  if (options.miniature) {
    ...
    html2canvas($(canvasContainer)[0]).then(function (canvas) {
      options.format =
        "image/" + (options.format != "pdf" ? options.format : "jpeg");
      ...
      $(canvasContainer).css({
        boxShadow: getCSS("box-shadow", "crowd-workspace-paper"),
      });
    });
  }
};
